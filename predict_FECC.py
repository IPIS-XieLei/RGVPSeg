...
setproctitle.setproctitle("Pre_FECC_TTFDP")
...
def predict1(img_dir, predict_t1, imgs_num, model1_path):
    ...
    # model1_path = "outputs2_clinic_t1/t1_92epoch_32batch.pth"
    # model2_path = "outputs2_clinic_fa/fa_52epoch_32batch.pth"
    # model3_path = "/home/ON_segmentation_A222/2D_FusionNet/outputs3_peaks/peaks_14epoch_32batch.pth"

    model1 = unet2d(4, 2).to(device)
    ...
    test_dataset = MyTrainDataset(ON_test_x_t1_dir, ON_test_x_t2_dir, ON_test_x_fa_dir, ON_test_x_dec_dir,
                                  ON_test_x_peaks_dir, ON_test_y_dir,
    ...
    with torch.no_grad():
        for x1, x2, x3, x4, x5, y in test_dataloader:
            ...
            input = torch.cat([inputs1, inputs4], 1)
...
def predict2(img_dir, predict_t1, imgs_num, model2_path):
    ...
    with torch.no_grad():
        for x1, x2, x3, x4, x5, y in test_dataloader:
            ...
            inputs4 = x4.to(device)
            inputs5 = x5.to(device)
            input = torch.cat((torch.cat([inputs3, inputs4], 1), inputs5), 1)
...
if __name__ == '__main__':
    ...
    weight1_path = "/home/ON_segmentation_A222/2D_FusionNet/outputs_FECC_t1dec/"
    ...
    for weights1 in weights1[0:100]:
        ...
        file_name = "predict_FECC_t1dec_fusion_metrics"
        ...
        mDice = '%.4f' % (mDice)
        print("mDice =", mDice)
        with open('predict_FECC_t1dec_fusion_metrics' + '/' + 'Dice.txt', 'a+') as f:
            f.writelines('{0}\t{1}\t\n'.format(filename, mDice))

    # weight2_path = "/home/ON_segmentation_A222/2D_FusionNet/outputs_FECC_fadecpeaks/"
    ...
    #
    #     file_name = "predict_FECC_fadecpeaks_fusion_metrics"
    #     if file_name not in os.listdir(os.curdir):
    #         os.mkdir(file_name)
    #
    #     # 预测结果保存
    ...
    #     mDice = '%.4f' % (mDice)
    #     print("mDice =", mDice)
    #     with open('predict_FECC_fadecpeaks_fusion_metrics' + '/' + 'Dice.txt', 'a+') as f:
    #         f.writelines('{0}\t{1}\t\n'.format(filename, mDice))
