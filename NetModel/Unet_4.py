...
class UNet2D_4(nn.Module):
    def __init__(self, in_channels=1, n_classes=2, is_deconv=True, is_batchnorm=True):
        ...
        self.center = unet2dConv2d(filters_base*4, filters_base*8, self.is_batchnorm)       # 512 1024
        # upsampling
    ...
    def forward(self, inputs):                  #  in->out       1*512*512      1*64*64
        ...
        center = self.center(maxpool3)          #  512->1024     1024*32*32     1024*4*4     unet2dConv2d-39:[-1, 1024, 4, 4]

        up3 = self.up_concat3(center, conv3)       #  512->256      256*128*128    256*16*16    unet2dUp-53:[-1, 256, 16, 16]
...
if __name__ == '__main__':
    ...
    print('#### Test Case ###')
    model = UNet2D_4(1,2).to(device)
    #summary(model, input_size=(1, 128, 160), batch_size=128)


    x = torch.rand(16, 1, 128, 160).to(device)
