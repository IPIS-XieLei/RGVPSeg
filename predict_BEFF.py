import config_2d
from NetModel import Unet_2d, Newnet,SAnet
import torch
...
import setproctitle
setproctitle.setproctitle("Pre_BEFF")
...
t1safaFuseUNet1=SAnet.t1safaFuseUNet1
...
def predict1(img_dir, predict_t1, imgs_num, model1_path):
    ...
    model1 = t1safaFuseUNet1(1,1).to(device)
    ...
    with torch.no_grad():
        for x1, x2, x3,x4,x5, y in test_dataloader:
            ...
            # input2 = torch.cat([torch.cat([inputs3, inputs4], 1), inputs5], 1)
...
def predict2(img_dir, predict_t1, imgs_num, model2_path):
    ...
    with torch.no_grad():
        for x1, x2, x3,x4,x5, y in test_dataloader:
            ...
            outputs,outputs1,outputs2 = model2(input1, input2)
            # outputs3 = model3(inputs3)
...
if __name__ == '__main__':
    ...
    weight1_path = "/home/ON_segmentation_A222/2D_FusionNet/outputs_sa_fr_t1_fa/"
    ...
    for weights1 in weights1[0:100]:
        ...
        file_name = "predict_sa_fr_t1_fa_fusion_metrics"
        ...
        mDice = '%.4f' % (mDice)
        print("mDice =", mDice)
        with open('predict_sa_fr_t1_fa_fusion_metrics' + '/' + 'Dice.txt', 'a+') as f:
            f.writelines('{0}\t{1}\t\n'.format(filename, mDice))

    # weight2_path = "/home/ON_segmentation_A222/2D_FusionNet/outputs_F2Net_tt_fdp/"
    ...
    #
    #     file_name = "predict_F2Net_tt_fdp_2_fusion_metrics"
    #     if file_name not in os.listdir(os.curdir):
    #         os.mkdir(file_name)
    #
    #     # 预测结果保存
    ...
    #     mDice = '%.4f' % (mDice)
    #     print("mDice =", mDice)
    #     with open('predict_F2Net_tt_fdp_2_fusion_metrics' + '/' + 'Dice.txt', 'a+') as f:
    #         f.writelines('{0}\t{1}\t\n'.format(filename, mDice))
